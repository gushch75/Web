...
def checkcandy(update: Update, context: CallbackContext):
    ...
    if(candys >0):
        ...
        SetMessageToUser(update,context,"Осталось шариков: "+ str(candys))
        if(flag == True):
            SetMessageToUser(update,context,update.effective_user.first_name + "ПОБЕДИЛ!")
        else:
            SetMessageToUser(update,context,"Бот ПОБЕДИЛ!")

def start_command(update: Update, context: CallbackContext):
    global candys
    candys = 21
    update.message.reply_text(f'Привет, {update.effective_user.first_name}! Начинаем игру!!!')
